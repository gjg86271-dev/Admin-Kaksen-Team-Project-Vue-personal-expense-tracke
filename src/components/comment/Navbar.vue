<template>
  <header class="navbar mx-2">
    ...
    <div class="navbar-right">
      ...
      <div ref="notifTriggerRef" style="position:relative">
        <button class="icon-button" type="button" aria-label="Notifications" @click="toggleNotif">
      ...
      <!-- Profile — ref moved directly onto the <button> (fixes positionMenu) -->
      <button class="profile" ref="triggerRef" @click="toggleDropdown" type="button">
        ...
        <svg class="chevron d-none d-sm-block" :class="{ open: isOpen }" width="14" height="14" viewBox="0 0 24 24">
  ...
  <Teleport to="body">
    <Transition name="panel">
      <div v-if="notifOpen" ref="notifMenuRef" class="notif-panel" :style="notifStyle">
    ...
    <Transition name="panel">
      <div v-if="isOpen" ref="menuRef" class="dropdown-menu" :style="menuStyle">
        ...
        <div class="dd-group">
          ...
          <!--
            NOTE: Use <span> for inner children so Vue scoped CSS applies.
            RouterLink renders as <a> — scoped child selectors work fine
            as long as we don't nest divs that escape the scope hash.
          -->
...
async function fetchBudgetAlerts() {
  ...
  try {
    ...
    const budgetRes = await api.get('budgets', { params: { month, year, limit: 100 } })
    const budgets   = (budgetRes.data?.data?.items ?? []).filter(
      b => b.month === month && b.year === year
    )
    if (!budgets.length) { alerts.value = []; return }

    const txRes = await api.get('transactions', {
      params: { month, year, type: 'EXPENSE', limit: 1000 }
    })
...
// ── Profile dropdown ───────────────────────────────────────────────────────
// FIX: ref is now directly on <button class="profile"> — no wrapper div
...
.icon-button {
  ...
  color: rgba(255,255,255,.75); transition: var(--transition); flex-shrink: 0;
}
.icon-button:hover {
...
.divider    { width: 1px; height: 28px; background: rgba(255,255,255,.2); }
...
.avatar-initials { color: #fff; font-weight: 700; font-size: 13px; }
.status-dot {
...
.profile-text  { display: flex; flex-direction: column; text-align: left; }
.profile-name  { font-size: 13px; font-weight: 600; color: #fff; line-height: 1.3; }
.profile-email { font-size: 11px; color: rgba(255,255,255,.6); line-height: 1.3; }
.chevron       { transition: transform .2s ease; color: rgba(255,255,255,.6); }
...
.notif-panel {
  ...
  overflow: hidden; display: flex; flex-direction: column; max-height: 460px;
}
.notif-head {
  display: flex; align-items: center; justify-content: space-between;
  padding: 13px 16px 11px; border-bottom: 1px solid var(--border-color); flex-shrink: 0;
}
...
.btn-mark-all {
  font-size: 11px; color: var(--color-primary); background: none; border: none;
  cursor: pointer; font-family: var(--font-khmer); padding: 0;
}
...
.progress-track {
  flex: 1; height: 5px; background: var(--border-color); border-radius: 999px; overflow: hidden;
}
...
.dd-avatar-img      { width: 100%; height: 100%; border-radius: 10px; object-fit: cover; }
.dd-avatar-initials { color: #fff; font-weight: 700; font-size: 15px; }
.dd-status-dot {
  position: absolute; bottom: -2px; right: -2px;
...
.dd-name      { font-size: 13px; font-weight: 700; color: var(--text-primary); line-height: 1.3; }
.dd-email     { font-size: 11px; color: var(--text-secondary); margin-top: 1px; }
.dd-role-badge {
  display: inline-flex; align-items: center; gap: 4px; margin-top: 5px;
  background: rgba(99,102,241,.18); border-radius: 6px;
  padding: 2px 8px; font-size: 10px; color: #818cf8; font-weight: 700;
}

.dd-group       { padding: 6px 0; }
.dd-group-label {
  font-size: 10px; color: var(--text-secondary);
  padding: 2px 14px 6px; letter-spacing: .06em; text-transform: uppercase;
}

/*
  KEY FIX: .dd-item works for both <button> and <a> (RouterLink).
  Vue scoped CSS adds a data-v-xxx attribute to elements in THIS component's template.
  RouterLink renders as <a> but the scoped attribute IS added to it since it is
  declared in this template. Children use <span> so they also get the attribute.
*/
.dd-item {
  display: flex; align-items: center; gap: 10px;
  padding: 9px 14px;
  ...
  font-size: 13px; font-family: var(--font-khmer);
...
.dd-item-text  { flex: 1; min-width: 0; display: inline-flex; flex-direction: column; }
.dd-item-label { font-size: 13px; line-height: 1.3; color: inherit; }
.dd-item-sub   { font-size: 10px; color: var(--text-secondary); margin-top: 1px; }

.dd-plan-badge {
  font-size: 10px; background: rgba(99,102,241,.2); color: #818cf8;
  border-radius: 6px; padding: 2px 8px; font-weight: 700; flex-shrink: 0;
}

.sep { height: 1px; background: var(--border-color); }

.dd-footer { padding: 10px 14px 12px; border-top: 1px solid var(--border-color); text-align: center; }
.dd-footer-meta { font-size: 10px; color: var(--text-secondary); }
