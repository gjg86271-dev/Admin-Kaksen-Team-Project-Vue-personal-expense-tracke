            ...
            <tbody>
              ...
              <template v-else>
                <tr class="text-center align-middle" v-for="(item, index) in filteredItems" :key="item.id">
                  ...
                  <td>
                    <button class="btn btn-sm action-btn-view" @click="openDetailModal(item)" title="មើល">
                      <i class="bi bi-eye"></i>
                    </button>
                    <button class="btn btn-sm action-btn-edit" @click="openModal(item)" title="កែប្រែ">
                      <i class="bi bi-pencil-square"></i>
                    </button>
                    <button class="btn btn-sm action-btn-delete" @click="confirmDelete(item)" title="លុប">
          ...
          <template v-else>
            <div v-for="item in filteredItems" :key="item.id" class="mobile-card">
              ...
              <div class="mobile-card__actions">
                <button class="btn btn-sm action-btn-view p-1" @click="openDetailModal(item)">
                  <i class="bi bi-eye"></i>
                </button>
                <button class="btn btn-sm action-btn-edit p-1" @click="openModal(item)">
                  <i class="bi bi-pencil-square"></i>
                </button>
                <button class="btn btn-sm action-btn-delete p-1" @click="confirmDelete(item)">
                  <i class="bi bi-trash"></i>
                </button>
              </div>
            </div>
      ...
      <Transition name="modal-fade">
        <div v-if="showModal" class="swal-overlay" @click.self="closeModal">
          <div class="swal-box">
            ...
            <!-- FORM LOADING SKELETON — uses same .skeleton class as page skeleton -->
            <template v-if="formLoading">
              <div class="swal-body">
                <div class="skeleton" style="height:14px; width:100px; border-radius:6px; margin-bottom:8px;"></div>
                <div class="skeleton" style="height:46px; border-radius:50px; margin-bottom:18px;"></div>
                <div class="skeleton" style="height:14px; width:80px; border-radius:6px; margin-bottom:8px;"></div>
                <div style="display:flex; gap:10px;">
                  <div class="skeleton" style="flex:1; height:46px; border-radius:12px;"></div>
                  <div class="skeleton" style="flex:1; height:46px; border-radius:12px;"></div>
                </div>
              </div>
              <div class="swal-footer">
                <div class="skeleton" style="flex:1; height:46px; border-radius:50px;"></div>
                <div class="skeleton" style="flex:2; height:46px; border-radius:50px;"></div>
              </div>
            </template>
...
async function saveCategory() {
  ...
  try {
    ...
    const result = isEditing.value && selectedItem.value?.id
      ? await categoryStore.updateCategory(selectedItem.value.id, { name: form.name.trim(), type: form.type })
...
async function doDelete() {
  ...
  try {
    ...
    await categoryStore.fetchCategories()
    showToast(
      result?.success !== false ? 'បានលុបប្រភេទដោយជោគជ័យ!' : (result?.message || 'លុបបានបរាជ័យ'),
...
/* ── Skeleton — shared base class used by BOTH page and form skeletons ── */
.skeleton {
...
.mobile-card-skeleton {
  height: 72px; border-radius: var(--radius); margin-bottom: 10px;
  background: linear-gradient(90deg, var(--bg-input) 25%, var(--border-color) 50%, var(--bg-input) 75%);
  background-size: 200% 100%;
  animation: shimmer 1.2s infinite;
}

/* swal-footer needs flex for skeleton items */
...
.form-loading-overlay {
  ...
  gap: 10px; font-size: 13px; color: var(--text-secondary); font-family: var(--font-khmer);
}
.form-spinner {
